"""Active exit management loop (Phase 1 fix for '0 closed trades').

The reconciler (reconcile.py) only *records* outcomes after a bracket leg
fills on its own. It cannot make a stuck position close. This loop runs each
cycle and ACTS: it moves stops to break-even at +1R and force-closes positions
that have hit the 10-trading-day time-stop.

The decision is delegated to the pure `exits.decide_exit`; this module only
talks to the broker. The broker dependency is a Protocol so the orchestration
is unit-tested with a fake broker (no Alpaca needed).

INTEGRATION NOTE (for live verification on the server): the concrete Broker
methods used here (get_current_price, get_open_stop, replace_stop, flatten)
wrap alpaca-py calls whose live behaviour — especially how a force-close
interacts with the still-open bracket legs — must be confirmed against the
paper account before trusting in production.
"""
...
def manage_open_positions(
    broker: ExitBrokerLike,
    store: DecisionStore,
    log=None,
    *,
    now: datetime | None = None,
) -> list[ManageAction]:
    """Apply break-even moves and time-stops to every open ENTER decision that
    has a broker_order_id. Returns the list of actions taken."""
    if log is None:
    ...
    for row in store.open_enter_decisions():
        ...
        if action.kind == exits.MOVE_BREAKEVEN and action.new_stop_price is not None:
            ...
            if fill is None:
                ...
                continue
            pnl = _pnl(float(row["plan_entry_price"]), fill.fill_price,
                       float(row["plan_qty"]), side)
            store.record_outcome(row["id"], exit_price=fill.fill_price,
                                 exit_reason="time_stop", pnl=pnl, closed_at=fill.filled_at)
            log.info("exit_time_stop_closed", decision_id=row["id"], symbol=symbol,
                     exit_price=fill.fill_price, pnl=pnl)
            actions.append(ManageAction(row["id"], symbol, action.kind, action.reason))
